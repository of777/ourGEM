...
models = ['single', 'independent', 'multimodal', 'icarl', 'ewc', 'gem', 'ourGem']
...
for dataset in datasets:
    ...
    plt.xticks([2, 9, 16], ['ACC', 'BWT', 'FWT'], fontsize=16)
    plt.yticks(fontsize=16)
    plt.xlim(-1, len(y_acc) * 3 + 2)
    plt.ylabel('classification accuracy', fontsize=16)
    plt.title(names_datasets[dataset], fontsize=16)
    plt.legend(fontsize=12, bbox_to_anchor=(1.05, 1), loc='upper left')  # 图例放在图外右侧
    plt.tight_layout()
